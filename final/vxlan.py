# for containers in Vxlan Network
import os
import sys
import random
cont1=sys.argv[1]
...
os.system("sudo ip link set dev v"+str(v1)+" netns ns2")
print("2")
os.system("sudo ip link set dev v"+str(v2)+" netns "+cont2id)
os.system("sudo nsenter -t "+cont2id+" -n ip link set v"+str(v2)+" up")
os.system("sudo ip netns exec ns2 ip link set v"+str(v1)+" up")
os.system("sudo ip netns exec ns2 brctl addif br3 v"+str(v1))
print("3")
#ip
ip1=random.randrange(2, 250, 1)
os.system("sudo nsenter -t "+cont2id+" -n ip addr add 12.1.0."+str(ip1)+"/24 dev v"+str(v2))
print("4")
#route
